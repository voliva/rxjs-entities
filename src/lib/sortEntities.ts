...
export function sortEntities<K, T>(
  ...
  /**
  switchMap is evil
  return compareFn$.pipe(
    switchMap((compareFn) =>
      combineKeys(keys$, getObservable$).pipe(
        scan((sortedKeys, mapWithChanges) => {
          const changes = Array.from(mapWithChanges.changes);
          const updatedKeys: K[] = [];
          const removedKeys: K[] = [];
          changes.forEach((key) => {
          ...
          return sortedKeys;
        }, new SortedKeyedArray<T, K>(compareFn)),
        map((sortedArray) => sortedArray.toArray())
      )
    )
  );
...
export function sortEntities<K, T>(
  ...
  return defer(() =>
    mergeWithKey({
      compareFn$,
      mapWithChanges$: combineKeys(
        keys$.pipe(tap(() => logTime("input"))),
        getObservable$
      ).pipe(
        tap(() => {
          logTime("combineKeys"); /// SLOW
        })
      )
    }).pipe(
      scan(
